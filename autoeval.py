...
def main(*, params="", root=".", batch_size=64, dataset="imagenet1k", dataset_root="/p/scratch/ccstdl/cherti1/clip_benchmark_datasets/imagenet1k", regexp="", amp=True, normalize=False, normalizer="microsoft/phi-2", normalize_type="add", output_file_format="{dataset}_{model}_{pretrained}.json"):
    ...
    for i, f in enumerate(files):
        ...
        cmd = f"python eval.py --pretrained {f} --model {model_name} --batch-size {batch_size} --dataset-name {dataset} --dataset-root {dataset_root} --output-file-format '{path}/{output_file_format}' --skip-existing {normalize_s} --normalizer {normalizer} --normalize-type {normalize_type} {amp_s}"
        cmds.append(cmd)
